...
function DataCenter({entries,brands,onAddMetric,onDeleteMetric,onDeleteMetrics}:any){
  ...
  const cancelBatchMode=()=>{setBatchMode(false);setSelectedIds(new Set())}
  const confirmBatchRemove=async()=>{const ids=[...selectedIds];if(!ids.length)return;setDeleting(true);try{await onDeleteMetrics(ids);setSelectedIds(new Set());setBatchDeleteOpen(false);setBatchMode(false)}catch(error){setFormError(error instanceof Error?error.message:'批量删除失败')}finally{setDeleting(false)}}
